...
print("===== DEBUG START: Check Excel assignments =====")
print("Row players (Zeilen):", row_players_links)
print("Col opponents (Spalten):", list(col_index_links.keys()))

for match_id, (player, opponent, switched_flag) in match_id_to_excel.items():
    try:
        print(f"\n➡️ Match {match_id}: player='{player}', opponent='{opponent}'")
        
        # Debug: prüfen, ob Player in row-Liste ist
        if player not in row_players_links:
            print(f"   ⚠️ Player '{player}' fehlt in row_players_links")
        else:
            row_idx = row_players_links.index(player) + 2
            print(f"   ✅ Player '{player}' gefunden in row {row_idx}")

        # Debug: prüfen, ob Opponent in col-Index ist
        if opponent not in col_index_links:
            print(f"   ⚠️ Opponent '{opponent}' fehlt in col_index_links")
        else:
            col_idx = col_index_links[opponent]
            print(f"   ✅ Opponent '{opponent}' gefunden in col {col_idx}")

            # Nur wenn beide gültig → aktuelle Cell prüfen
            if player in row_players_links:
                row_idx = row_players_links.index(player) + 2
                cell = ws_links.cell(row=row_idx, column=col_idx)
                print(f"   📊 Cell ({row_idx}, {col_idx}) before='{cell.value}'")
                if not cell.value:
                    cell.value = str(match_id)
                    print(f"   ✍️ Wrote match ID {match_id} into cell")
                else:
                    print(f"   ⏩ Cell already filled with '{cell.value}'")

    except Exception as e:
        print(f"   ❌ Error while assigning match {match_id}: {e}")
